"""The schema exactly as version 7 created it, retained as a migration fixture.

`test_schema_parity` builds a version-7 database from this text, migrates it to 8, and
compares it against a freshly-initialised 8. Without a retained copy there is nothing to
migrate *from* once `engine/schema.py` has been edited in place.

It lives here and **not** in `engine/schema.py` on purpose. `tests/test_schema_vocabulary.py`
regexes every `CREATE TABLE IF NOT EXISTS` out of that whole file, so a retained older DDL
sitting there would be phantom schema for all five of its checks: it declares `subtasks` and
`package_id`, and the suite would certify the presence of the very words schema 8 removes.

Generated from `git show <v7>:engine/schema.py` rather than hand-copied — a hand-copy is a
second source of truth for a text whose whole job is to be the first one.
"""
