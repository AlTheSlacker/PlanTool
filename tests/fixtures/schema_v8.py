"""The schema exactly as version 8 created it, retained as a migration fixture.

`test_schema_parity` builds a version-8 database from this text, migrates it to 9, and
compares it against a freshly-initialised 9. Without a retained copy there is nothing to
migrate *from* once `engine/schema.py` has been edited in place.

It lives here and **not** in `engine/schema.py`, for the same reason `schema_v7` does.
`tests/test_schema_vocabulary.py` regexes every `CREATE TABLE IF NOT EXISTS` out of that
whole file, so a retained older DDL sitting there would be phantom schema for every one of
its checks — including this change's own `JUSTIFICATION_ROLES` count, which would read
`plan_rows` twice and count the same columns again.

Generated from the live `engine/schema.py` at schema 8 rather than hand-copied — a
