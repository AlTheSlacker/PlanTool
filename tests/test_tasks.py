...
def test_finalize_derives_one_subtask_per_contract(tasks, rows):
    ...
    assert {str(s.contract_ref) for s in graph.subtasks} == {
...
def test_finalize_orders_dependencies_first(tasks, rows):
    """requirements:34 — no sub-task precedes its dependencies."""
    graph = _finalize(tasks, rows, n=3, deps=[(0, 1), (1, 2)])
    ids = {str(s.contract_ref): s.id for s in graph.subtasks}
...
def test_finalize_blocks_on_unresolved_findings(tasks, rows, findings):
    ...
    findings.file_finding([ref], "the gate is too weak", "high")
...
def test_readiness_is_derived_not_stored(tasks, rows, store):
    """D10 — `ready` is never written to subtasks.state."""
    _finalize(tasks, rows, n=1)
    assert store.query("SELECT state FROM subtasks")[0]["state"] == PENDING
    assert tasks.readiness_of(tasks.get(1)) == READY


def test_a_subtask_with_unfinished_deps_is_not_ready(tasks, rows):
...
def test_rework_flagged_becomes_ready_again(tasks, rows):
    ...
    sub-task's `deps_satisfied` edge has already fired for the last time (its
...
def test_next_subtask_does_not_start_the_work(tasks, rows):
    """Candidacy is not delivery. A sub-task may be offered and never briefed; marking
...
def test_report_status_names_the_missing_subtask(tasks, rows):
    _finalize(tasks, rows, n=1)
    with pytest.raises(SubTaskNotFound):
...
def test_a_verdict_cannot_be_banked_before_the_work_is_served(tasks, rows):
    ...
    be recorded against a `pending` sub-task and satisfy contracts:60's guard later:
...
# --- contracts:38 graph_status / contracts:55 next_subtask ---
...
def test_next_subtask_refuses_a_draft_plan(tasks, rows):
...
def test_allow_draft_has_nothing_to_serve_before_finalization(tasks, rows):
    ...
    PlanNotFinalized, but sub-tasks are derived *at* finalization (crud_grid:33), so a
    ...
    with pytest.raises(PlanNotFinalized) as exc:
        tasks.next_subtask(allow_draft=True, consent="owner said go, 2026-07-21")
    assert "derived at finalization" in str(exc.value)


def test_next_subtask_names_the_blockers_rather_than_serving(tasks, rows):
    """uc_extensions:34 — never serve unbuildable work."""
    _finalize(tasks, rows, n=2, deps=[(0, 1)])
    tasks.serve_brief(2)
    report = tasks.next_subtask()
    assert bool(report) is False
    assert report.blocking[1] == (2,)


def test_next_subtask_returns_the_closure_not_a_brief(tasks, rows):
