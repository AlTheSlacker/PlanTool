...
| 8 | Is an **entity's** state stored or derived from its history? | Stored as a column, with the transition table as data rather than code. This is about an entity's own lifecycle. **Derived reports — gaps, warnings, the task graph — are computed at read and stored nowhere**, which is why closing a gap is a matter of writing the thing it asked for rather than of clearing a flag. |
