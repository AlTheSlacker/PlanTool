# Package 8 — Finalization (verify mode)

Planning ends here, mechanically. No new content belongs in this package — if the render or
the gate turns up a content problem, that is a package-1..7 fix first.
...
2. `run_gate(8)` — two criteria, and they are the whole gate: every earlier package's gate
