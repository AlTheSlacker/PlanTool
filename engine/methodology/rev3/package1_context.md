# Package 1 — Context & goals (elicit mode)
...
- Each actor: an `actors` row, its `name` the actor. Every actor needs a scenario eventually
  (the package-2 cross-check enforces it), so record only those who touch or are affected by
  the system.
...
- Constraints often make good ubiquitous/state `requirements` rows; core domain nouns can be
  filed early into `entities`. File them now; they are gated in packages 3 and 4.
