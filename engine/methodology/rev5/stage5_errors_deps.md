...
- **`dependencies`**: every service, API, library, filesystem,
  or queue outside the planned system's control. If there are genuinely none, file a
  `no_dependencies_decision` row saying so and why — that is the table the gate reads, and
  it accepts nothing less explicit.
